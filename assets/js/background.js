...
const EIGHT = [
    ...
    [
        step + Math.cos(ne) * 0.065,
        edge + Math.sin(ne) * 0.065,
        step + Math.cos(sw) * halfWidth,
        edge + Math.sin(sw) * halfWidth
    ],

    [
        step + Math.cos(ne) * halfWidth,
        -edge + Math.sin(ne) * halfWidth,
        step + Math.cos(sw) * 0.065,
        -edge + Math.sin(sw) * 0.065
    ],
]
...
void main() {
  ...
  uv += ripple(vec4(uv, uv), 0., pow(anim, 50.) * 15. + sin(time * 0.5), 0.05, time);
  ...
  ${lines.map((line, i) => {
      ...
      if (i > 0.0) {
        fgColor = mix(bgColor, fgColor, smoothstep(-0.009, -0.001, l));
      }
    `;
    }).join(`
  `)}
